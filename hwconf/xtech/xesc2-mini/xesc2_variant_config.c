...
// ------------------------------------------------------------------
// Test mode: Use a static const OTP block instead of reading hardware
// Set to 1 to use the hardcoded test block below, 0 for real OTP.
// ------------------------------------------------------------------
#define XESC2_USE_TEST_OTP 1

#if XESC2_USE_TEST_OTP
// Generated by: otp_brand -key xesc-firmware.priv -hw 2.0.0 -type mini -variant v2_power --dry-run --dump-c
// Board: mini, Variant: v2_power, HW: 2.0.0, Serial: 1, Timestamp: 1781294597, CRC16: 0x893B

// Block 0 — Data + CRC16 + Signature Part 1 (16 bytes)
static const uint8_t test_otp_block[XESC2_OTP_BLOCK_SIZE] = {
    0xEC,
    0x01,
    0x00,
    0x02,
...
// Block 1 — Signature Part 2 (32 bytes)
// Reserved for future firmware-side signature verification.
static const uint8_t test_otp_sig_block2[XESC2_OTP_BLOCK_SIZE] = {
    0x99,
    0x82,
    0x07,
    0x06,
...
// ------------------------------------------------------------------
// OTP scanner: find the last valid block pair
// ------------------------------------------------------------------
static const uint8_t *otp_read_block(uint8_t pair_index)
{
#if XESC2_USE_TEST_OTP
    (void)pair_index;
    return test_otp_block;
#else
...
static xesc2_otp_identity_t otp_scan(void)
{
    ...
    // Scan block pairs backwards to find the last valid one
